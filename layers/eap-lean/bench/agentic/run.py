#!/usr/bin/env python3
"""Agentic, multi-file benchmark for eap-lean.
...
eap-lean: the claude CLI is the harness (already installed, we run inside it). No SDK
...
ARMS = {
    ...
    "eap-lean":       lambda: _skill("layers/eap-lean/EAP-LEAN.md"),
...
# EAP-Lean is rule text (EAP-LEAN.md), not a Claude marketplace plugin — always
# --append-system-prompt. Empty PLUGIN_ARMS keeps the optional plugin path unused.
PLUGIN_ARMS = ()
PLUGIN_CACHE = Path.home() / ".claude" / "plugins" / "cache"

def _plugin_dir(name):
    """Resolve a plugin's cache dir portably. Env override key uses underscores
    (eap-lean → EAP_LEAN_PLUGIN_DIR). Order: env → latest cache dir → clear error.
    Unused while PLUGIN_ARMS is empty (EAP-Lean appends rule text); kept for optional
    plugin-arm experiments."""
    env_key = f"{name.replace('-', '_').upper()}_PLUGIN_DIR"
...
# tests is still explicitly allowed, so eap-lean's "leave a runnable check" discipline is not suppressed.
...
def _selfcheck_split(p: Path):
    ...
    check is the runnable check eap-lean's rule asks for -- a positive signal, not source bloat --
...
def code_stats(workdir: Path, selfcheck_as_test: bool = False):
    ...
    reclassified from source to test, so following eap-lean's 'leave a runnable check' rule is not
...
def _selftest_plugin_dir():
    ...
    fails = 0
    sentinel = "/tmp/eap-lean-selftest-plugin-dir"
    os.environ["EAP_LEAN_PLUGIN_DIR"] = sentinel
    try:
        ok_env = _plugin_dir("eap-lean") == sentinel
    finally:
        del os.environ["EAP_LEAN_PLUGIN_DIR"]
    print(f"{'ok ' if ok_env else 'XX '} plugin_dir   env  override honored")
    fails += 0 if ok_env else 1
    missing = "eap-lean-does-not-exist-xyz"          # no env, no cache entry -> must sys.exit
    try:
