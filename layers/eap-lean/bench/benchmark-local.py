"""
EAP-Lean local benchmark — runs the same 5 tasks against any Ollama model.
No promptfoo or API key required. Compares baseline vs eap-lean on code LOC
and wall-clock time. Results are printed as a table and saved to a JSON file.

Usage:
    python layers/eap-lean/bench/benchmark-local.py
...
LAYER = Path(__file__).parent.parent  # layers/eap-lean/
...
def load_arms():
    return {
        "baseline": None,
        "eap-lean": (LAYER / "EAP-LEAN.md").read_text(encoding="utf-8"),
    }


def count_loc(text):
    """Non-blank, non-comment lines of code: fenced blocks, or the whole
    response when the model emitted bare code with no fence. Mirrors loc.cjs."""
...
def run(model, repeat, ollama_url):
    ...
    # results[arm][task_id] = list of {loc, time, response}
    ...
    arm_total = sum(med_loc["eap-lean"][t] for t in task_ids)
    pct = (1 - arm_total / base_total) * 100 if base_total else 0
    sign = "less" if pct >= 0 else "more"
    print(f"  eap-lean  : {arm_total} LOC  ({abs(pct):.0f}% {sign} than baseline)")
...
def main():
    parser = argparse.ArgumentParser(description="EAP-Lean local benchmark via Ollama")
